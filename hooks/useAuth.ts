  ...
  const loadAuthFromStorage = useCallback(async () => {
    ...
    try {
      ...
      debugLog("localStorage 값:", {
        tokensJson: tokensJson ? tokensJson : "null",
        userJson: userJson ? userJson : "null",
        tokensContent: tokensJson ? tokensJson.substring(0, 100) + "..." : null,
        userContent: userJson ? userJson.substring(0, 100) + "..." : null,
      });
      ...
      if (tokensJson && userJson) {
        try {
          ...
          debugLog("토큰 및 사용자 데이터 파싱 완료", {
            user,
            tokens,
            tokenKeys: Object.keys(tokens),
            hasAccess: "access" in tokens,
            hasRefresh: "refresh" in tokens,
            accessValue: tokens.access ? "exists" : "missing",
            refreshValue: tokens.refresh ? "exists" : "missing",
          });
            ...
            try {
              ...
              debugLog("토큰 갱신 성공:", {
                access: newTokens.access
                  ? newTokens.access.substring(0, 20) + "..."
                  : "undefined",
                refresh: newTokens.refresh
                  ? newTokens.refresh.substring(0, 20) + "..."
                  : "undefined",
              });
  ...
  const login = async (
    ...
    try {
      ...
      if (response.success) {
        ...
        const userTypeCamel =
          (response as any).userType || (response as any).user_type;
        const userData =
          userTypeCamel === "designer"
            ? (response as any).designer
            : (response as any).factory;

        debugLog("추출된 사용자 데이터:", {
